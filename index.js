...
await interaction.channel.permissionOverwrites.set([
  ...
  // ✅ BOT ITSELF (THIS FIXES EVERYTHING)
  {
...
if (cmd === "unbye") {
  ...
  try {
   ...
   // ⚡ zap = quick kick
    ...
    // get mention OR ID
const arg = args[0];
if (!arg) return message.reply("❌ Mention a user or provide a user ID.");

// try mention first
let target =
  message.mentions.members.first() ||
  await message.guild.members.fetch(arg).catch(() => null);

if (!target) {
  return message.channel.send("❌ User not found (invalid mention or ID).");
}
...
await target.ban({
  reason: `Bye by ${message.author.tag}`,
});

await message.channel.send(
  `🚫 ${target.user.tag} said **BYE NIGGERS SUCK MY DICKKKKKK FUCKING DICKS**`
);
});
